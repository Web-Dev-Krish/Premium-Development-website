...
// Uses plain fetch against the Resend REST API, so no extra dependency is needed.
//
// Required env vars (set these in Vercel → Project → Settings → Environment Variables):
//   RESEND_API_KEY   - your Resend API key (starts with "re_")
//   RESEND_FROM_EMAIL - the verified "from" address, e.g. "Devsiy <leads@yourdomain.com>"
//                        On Resend's free tier, until you verify your own domain you can use
//                        "onboarding@resend.dev" as the from address for testing.
//
// Who receives the email is controlled from the Admin Panel → Site Settings →
// "Lead Notification Emails" field (comma-separated), stored in the `site_settings` table
// under the key `notification_emails`. This lets you add/remove recipients (e.g. your
// co-founder) without touching code or redeploying.
...
export async function sendLeadNotificationEmail(lead) {
  try {
    ...
    // Never let an email failure break lead submission.
