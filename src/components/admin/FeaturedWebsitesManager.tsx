...
export default function FeaturedWebsitesManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { ...form };
    if (editing) {
      await adminFetch('/api/featured-websites', { method: 'PUT', body: JSON.stringify({ id: editing.id, ...payload }) });
    } else {
      await adminFetch('/api/featured-websites', { method: 'POST', body: JSON.stringify(payload) });
    }
    reset();
    fetchData();
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { reset(); setShowForm(true); }} className="flex items-center gap-2 px-4 py-2 bg-white text-neutral-950 rounded-lg text-sm hover:bg-neutral-200 transition-colors">
          <Plus className="w-4 h-4" /> Add Featured Website
        </button>
      </div>
      ...
      {showForm && (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-white font-light">{editing ? 'Edit Featured Website' : 'New Featured Website'}</h3>
            <button type="button" onClick={reset} className="text-neutral-500 hover:text-white"><X className="w-4 h-4" /></button>
          </div>
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            <input required placeholder="Website Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="bg-neutral-900 border border-white/10 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:border-white/30" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div key={item.id} className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden flex flex-col justify-between">
              <div className="p-2">
                <LivePreview url={item.project_url} title={item.title} aspectRatio="aspect-video" />
              </div>
