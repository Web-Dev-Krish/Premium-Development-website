          ...
          {founders.map((founder) => (
            <div key={founder.id} className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 flex gap-4">
              <div className="w-20 h-20 rounded-full overflow-hidden border border-white/10 shrink-0">
                <SafeImage src={founder.image_url} alt="" className="w-full h-full object-cover" fallback={<div className="w-full h-full bg-neutral-800 flex items-center justify-center text-neutral-500 text-2xl font-light">{founder.name?.charAt(0)}</div>} />
              </div>
